package com.cxc.firstproject.model;
...
/**
 * Created by jingbin on 2016/12/1. 每日推荐model
 */
...
public class EverydayModel {
    ...
    public void showBanncerPage(final RequestImpl listener) {
        Subscription subscription = HttpClient.Builder.getTingServer()
                .getFrontpage().observeOn(AndroidSchedulers.mainThread())
    ...
    public void showRecyclerViewData(final RequestImpl listener) {
        SPUtils.put(HOME_ONE, "");
        SPUtils.put(HOME_TWO, "");
        SPUtils.put(HOME_SIX, "");
        Func1<GankIoDayBean, Observable<List<List<AndroidBean>>>> func1 = new Func1<GankIoDayBean, Observable<List<List<AndroidBean>>>>() {
            @Override
            public Observable<List<List<AndroidBean>>> call(
                    GankIoDayBean gankIoDayBean) {
        ...
        Subscription subscription = HttpClient.Builder.getGankIOServer()
                .getGankIoDay(year, month, day).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).flatMap(func1)
                .subscribe(observer);
        listener.addSubscription(subscription);
    }

    // subList没有实现序列化！缓存时会出错！
    private void addUrlList(List<List<AndroidBean>> lists,
            List<AndroidBean> arrayList, String typeTitle) {
        // title
    ...
    private AndroidBean getAndroidBean(List<AndroidBean> arrayList, int i,
            int androidSize) {
        ...
        // 随机图的url
        if (i < 3) {
            androidBean.setImage_url(
                    ConstantsImageUrl.HOME_SIX_URLS[getRandom(3)]);// 三小图
        } else if (androidSize == 4) {
            androidBean.setImage_url(
                    ConstantsImageUrl.HOME_ONE_URLS[getRandom(1)]);// 一图
        } else if (androidSize == 5) {
            androidBean.setImage_url(
                    ConstantsImageUrl.HOME_TWO_URLS[getRandom(2)]);// 两图
    ...
    private List<AndroidBean> addUrlList(List<AndroidBean> arrayList,
            int androidSize) {
        ...
        for (int i = 0; i < androidSize; i++) {
            ...
            if (androidSize == 1) {
                ...
                androidBean.setImage_url(
                        ConstantsImageUrl.HOME_SIX_URLS[getRandom(3)]);// 三图
            }
    ...
    private int getRandom(int type) {
        ...
        if (!TextUtils.isEmpty(home_six)) {
            ...
            for (int j = 0; j < urlLength; j++) {
                ...
                for (String aSplit : split) {
                    if (!TextUtils.isEmpty(aSplit)
                            && String.valueOf(randomInt).equals(aSplit)) {
                        isUse = true;
                        break;
                    }
                }
                if (!isUse) {
                    ...
                    SPUtils.put(saveWhere, sb.toString());
                    return randomInt;
                }
            }
            ...
            Random random = new Random();
            int randomInt = random.nextInt(urlLength);
            SPUtils.put(saveWhere, randomInt + ",");
            return randomInt;
        }
        return 0;
    }
